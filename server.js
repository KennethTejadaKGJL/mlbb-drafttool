...
io.on('connection', (socket) => {
  ...
  socket.on('hero_hover', ({ hero, team }) => {
    ...
    gameState.tempHover = hero;
    io.emit('update_state', gameState);
  });

  socket.on('confirm_lock', () => {
    if (gameState.finished || !gameState.started || !gameState.tempHover) return;

    const currentStep = DRAFT_ORDER[gameState.stepIndex];

    const team = currentStep.team;
    // 1. Strict Turn Validation (Already handled by sender logic, but good double check)
    // if (team !== currentStep.team) return;

    // 2. Logic to update state
    if (currentStep.type === 'ban') {
      if (team === 'blue') gameState.blueBans.push(gameState.tempHover);
      else gameState.redBans.push(gameState.tempHover);
    } else {
      if (team === 'blue') gameState.blueTeam.push(gameState.tempHover);
