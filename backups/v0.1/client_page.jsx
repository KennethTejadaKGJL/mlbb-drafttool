...
export default function DraftBoard() {
  ...
  const handleHeroClick = (hero) => {
    ...
    socket.emit('hero_selected', { hero, team: teamToSubmit });
  };
  ...
  return (
    <div className="bg-[#01040a] h-screen w-screen text-white font-sans overflow-hidden flex flex-col select-none italic">
      {/* HEADER */}
      <div className="flex justify-between items-center px-8 bg-slate-900 border-b border-white/5 h-[12vh] shadow-2xl relative z-50">
        <div className="flex gap-2">
          {[0, 1, 2, 3, 4].map(i => (
            ...
            `}>
              ...
              {gameState.blueBans[i] && <Image src={getImgPath(gameState.blueBans[i].image)} alt={`Blue Ban ${i + 1}`} fill className="object-cover object-top grayscale opacity-40 animate-in fade-in zoom-in duration-500" sizes="(max-width: 768px) 10vw, 5vw" />}
        ...
        <div className="flex gap-2">
          {[0, 1, 2, 3, 4].map(i => (
            ...
            `}>
              ...
              {gameState.redBans[i] && <Image src={getImgPath(gameState.redBans[i].image)} alt={`Red Ban ${i + 1}`} fill className="object-cover object-top grayscale opacity-40 animate-in fade-in zoom-in duration-500" sizes="(max-width: 768px) 10vw, 5vw" />}
      ...
      <div className="flex flex-1 overflow-hidden h-full">
        ...
        {/* BLUE SIDEBAR */}
        <div className="w-[30%] flex flex-col bg-slate-950 h-full relative z-10 border-r border-white/5 shadow-[20px_0_50px_rgba(0,0,0,0.5)]">
          {[0, 1, 2, 3, 4].map(i => (
            ...
            `}>
              ...
              {gameState.blueTeam[i] ? (
                <div className="h-full w-full relative animate-in fade-in slide-in-from-left-12 duration-1000">
        ...
        `}>
          <div className="p-6 pb-2 space-y-4 relative z-30 bg-[#01040a]">
            <div className="flex items-center gap-4 h-[45px]">
              <div className="flex-1 relative h-full">
                <span className={`absolute -top-2 left-3 bg-[#01040a] px-2 text-[10px] font-bold uppercase tracking-widest z-10 ${isPanicTime ? 'text-red-500' : 'text-slate-500'}`}>
                  {isPanicTime ? '⚠️ SELECTION REQUIRED' : 'Hero Database'}
                </span>
                <input className="bg-slate-900/50 border border-slate-800 rounded-md px-4 h-full text-xs w-full focus:outline-none focus:border-white/20 text-white font-bold tracking-widest uppercase" placeholder="SEARCH HERO..." onChange={(e) => setSearch(e.target.value.toLowerCase())} />
              </div>
              {isAdmin && (
                <div className="flex gap-2 h-full">
                  {!gameState.started && <button onClick={handleStart} className="h-full px-4 bg-emerald-600/10 hover:bg-emerald-600/20 text-emerald-500 text-[10px] font-black border border-emerald-500/20 rounded-md uppercase transition-all shadow-[0_0_10px_rgba(16,185,129,0.2)]">Start Draft</button>}
                  <button onClick={handleReset} className="h-full px-4 bg-red-600/10 hover:bg-red-600/20 text-red-500 text-[10px] font-black border border-red-500/20 rounded-md uppercase transition-all">Reset</button>
                </div>
              )}
            </div>
            <div className="space-y-1.5">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.3em] pl-1">Role Classification</span>
              <div className="flex gap-1 justify-between">
                {['All', 'Tank', 'Fighter', 'Assassin', 'Mage', 'Marksman', 'Support'].map(r => (
                  <button key={r} onClick={() => setRoleFilter(r)} className={`flex-1 py-1.5 rounded-sm text-[10px] font-black uppercase transition-all tracking-tighter border ${roleFilter === r ? 'bg-white text-black border-white shadow-xl' : 'bg-slate-900 border-slate-800 text-slate-600 hover:text-slate-300'}`}>{r}</button>
                ))}
              </div>
            </div>
          </div>
          <div className="flex-1 overflow-y-auto px-6 custom-scrollbar pb-10 relative z-30">
            <div className="grid grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-3">
              {heroesData.filter(h => (roleFilter === 'All' || h.role.includes(roleFilter)) && h.name.toLowerCase().includes(search)).map(hero => {
                ...
                return (
                  <button key={hero.id} onClick={() => handleHeroClick(hero)} disabled={isTaken || !isMyTurn} className={`relative aspect-square rounded-sm overflow-hidden border transition-all duration-300 ${isTaken ? 'grayscale opacity-5 scale-90 pointer-events-none' : 'border-white/5 hover:border-yellow-500/50 hover:scale-110'} group`}>
                    <Image src={getImgPath(hero.image)} alt={hero.name} fill className="object-cover" sizes="(max-width: 768px) 25vw, 15vw" />
                    {!isTaken && (
                      /* UPDATED: Bigger, bolder hero labels with higher contrast */
                      <div className="absolute bottom-0 w-full bg-slate-950/90 text-[10px] md:text-[11px] font-black text-center py-1 truncate px-1 text-slate-100 uppercase tracking-tighter border-t border-white/10 group-hover:text-yellow-400">
                        {hero.name}
                      </div>
                    )}
                  </button>
        ...
        {/* RED SIDEBAR */}
        <div className="w-[30%] flex flex-col bg-slate-950 h-full relative z-10 border-l border-white/5 shadow-[-20px_0_50px_rgba(0,0,0,0.5)]">
          {[0, 1, 2, 3, 4].map(i => (
            <div key={i} className={`flex-1 relative overflow-hidden border-b border-white/5 transition-all duration-500
            ...
            `}>
              ...
              {gameState.redTeam[i] ? (
                <div className="h-full w-full relative animate-in fade-in slide-in-from-right-12 duration-1000">
